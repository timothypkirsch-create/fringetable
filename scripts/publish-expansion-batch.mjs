...
const recipeHref=u=>{
 ...
 if(external(value)||/^(?:mailto:|tel:|#|javascript:)/i.test(value))return value;
 return /^(?:assets|collections|guides|regions|subrecipes)\//.test(value)?`../${value}`:value;
};
...
let pronunciationJs=await fs.readFile('assets/js/pronunciation.js','utf8');
const pronunciationTerms=new Set(
 [...pronunciationJs.matchAll(/\['((?:\\'|[^'])+)'/g)]
   .map(m=>m[1].replace(/\\'/g,"'").toLowerCase())
);
const quotePronunciation=s=>`'${String(s).replace(/\\/g,'\\\\').replace(/'/g,"\\'")}'`;
const pronunciationRows=[];
for(const r of added){
 const term=String(r.pronunciationTerm||r.name).toLowerCase();
 if(pronunciationTerms.has(term))continue;
 pronunciationRows.push(`[${[
   r.pronunciationTerm||r.name,
   r.pronunciation,
   r.pronunciationRegion,
   r.pronunciationDefinition
 ].map(quotePronunciation).join(',')}],`);
 pronunciationTerms.add(term);
}
if(pronunciationRows.length){
